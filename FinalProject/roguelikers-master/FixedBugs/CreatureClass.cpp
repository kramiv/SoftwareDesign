...
class Hero: public creature{  //Subclass for hero specific variables and functions
    ...
    int maxhealth;
public:
    void PSetup(int l, int t, int h, int mh, int a, int g){
        left = l;
        top = t;
        health = h;
        maxhealth = mh;
    ...
    void PrintHealth(int h, int y){ //Does the same thing as print gold above, except for health
        int x = h%10;
        const char c = PrintNumber(x);
        if (h > 10){
            PrintHealth(h/10, y - 1);
        }
        _put_char(y, c);
    }
    void CheckHealth(int h){ //Does the same thing as check gold above, except for health
        int x;
        if (h < 10){
            x = 1;
            PrintHealth(h, x - 1);
...
void SetUpRoom(){
    ...
    if(just_starting()){ //initializes the player and enemy locations location
        ...
        Player.PSetup(_get_int(400),_get_int(410),_get_int(500),_get_int(650),_get_int(510),_get_int(600));
    ...
    _put_int(650, Player.GetMaxHealth());
